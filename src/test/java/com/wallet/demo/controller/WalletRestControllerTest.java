...
public class WalletRestControllerTest {
	...
	@Tag("DEV")
	@Test
	@DisplayName("Test - getAll() - /wallet/all/ - Failure Get All Wallet")
	void testGetAllWallet_Failure() throws Exception {
		// TDD Execute the GET request failure test
		mockMvc.perform(get("/wallets/all/").contentType(MediaType.APPLICATION_JSON))
	...
	@Tag("DEV")
	@Test
	@DisplayName("Test - payCoinFromCoinList() - /{id}/{paycoin} - Expect HTTP Response 404 if omit put wallet Id")
	void testPayCoinFromCoinList_ExpectHttp404Response() throws Exception {
		...
		mockMvc.perform(put("/wallet/1/").contentType(MediaType.APPLICATION_JSON).content(asJsonString(wallet)))
				// Validate the response code and content type
				.andExpect(status().isOk());
	}
	...
	@Tag("DEV")
	// @Test
	@DisplayName("Test - payCoinFromCoinList() - /{id}/{paycoin} - Expect HTTP Response 500 if omit coinList")
	void testPayCoinFromCoinList_ExpectHttp500Response() throws Exception {
		...
		mockMvc.perform(put("/wallet/1/1/").contentType(MediaType.APPLICATION_JSON).content(asJsonString(wallet)))
				// Validate the response code and content type
	...
	@Tag("DEV")
	@Test
	@DisplayName("Test - payCoinFromCoinList() - /{id}/{paycoin}  - Successful Payment")
	void testPayCoinFromCoinList_SuccessPayment() throws Exception {
		...
		wallet.setCoinList(mockCoinList);
		doReturn(wallet).when(service).getById(1L);
		doReturn(changeCoinList).when(service).computeCoinPayment(1, mockCoinList);
		...
		verify(service, times(1)).computeCoinPayment(1, mockCoinList);
		verify(service, times(1)).createOrUpdate(isA(Wallet.class));
	}
